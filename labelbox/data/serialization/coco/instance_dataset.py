...
def mask_to_coco_object_annotation(
    annotation: ObjectAnnotation, annot_idx: int, image_id: int, category_id: int
...
def rle_to_common(
    ...
    return ObjectAnnotation(
        name=class_name, value=Mask(mask=MaskData.from_2D_arr(mask), color=[1, 1, 1])
    )
...
class CocoInstanceDataset(BaseModel):
    ...
    def from_common(cls, labels: LabelCollection, image_root: Path, max_workers=8):
        ...
        categories = [
            Categories(
                id=category_mapping[idx], name=name, supercategory="all", isthing=1
            )
