    ...
    def validate_bounds_lat_lng(cls, values):
        ...
        if epsg == EPSG.EPSG4326:
            for bound in bounds:
                ...
                if int(lng) not in VALID_LNG_RANGE or int(lat) not in VALID_LAT_RANGE:
                    raise ValueError(
...
class TiledImageData(BaseData):
    ...
    def asdict(self) -> Dict[str, str]:
        return {
            ...
            "alternativeLayers": [layer.asdict() for layer in self.alternative_layers],
    ...
    def raster_data(
        ...
        rounded_tiles, pixel_offsets = list(
            zip(*[self._tile_to_pixel(pt) for pt in [xstart, ystart, xend, yend]])
        )
    ...
    def value(self) -> np.ndarray:
        ...
        return self.raster_data(self.zoom_levels[0], multithread=self.multithread).value

    def _get_simple_image_params(self, zoom) -> Tuple[float, float, float, float]:
        ...
        return (
            *[x * (2 ** (zoom)) / self.tile_size for x in [xstart, ystart, xend, yend]],
        )
    ...
    def _latlng_to_tile(self, lat: float, lng: float, zoom=0) -> Tuple[float, float]:
        ...
        for y in range(y_tile_start, y_tile_end + 1):
            ...
            for x in range(x_tile_start, x_tile_end + 1):
                try:
                    ...
                    row.append(
                        np.zeros(
                            shape=(self.tile_size, self.tile_size, 3), dtype=np.uint8
                        )
                    )
    ...
    def _validate_num_tiles(
        self, xstart: float, ystart: float, xend: float, yend: float, max_tiles: int
    ):
...
class EPSGTransformer(BaseModel):
    ...
    @classmethod
    def geo_and_pixel(
        cls, src_epsg, pixel_bounds: TiledBounds, geo_bounds: TiledBounds, zoom=0
    ) -> Callable:
        ...
        # handles 3857 from meters
            ...
            def transform(x: int, y: int) -> Callable[[int, int], Transformer]:
                point_in_px = PygeoPoint.from_meters(meter_y=y, meter_x=x).pixels(zoom)
    ...
    def __call__(
        ...
        if isinstance(shape, Polygon):
            return Polygon(points=[self._get_point_obj(p) for p in shape.points])
